...
public class TicketManager {
    ...
    public void create() {
        ...
        Set<String> tagsSet = new HashSet<>(Arrays.asList(tags.toLowerCase().split(",")));
    ...
    public void update()
    {
        ...
        if(ticketService.isTicketExist(id)){
            ...
            String agentName = InputReader.readString();
            System.out.println("Enter comma separated tags of ticket.(Click enter to continue or enter 'clear' for remove all tags)");
            String tags = InputReader.readString();
            Set<String>  tagSet = null;
            if(TicketUtil.isValidString(tags)) {
                if(tags.equals("clear"))
                    tagSet = new HashSet<>();
                else
                    tagSet = new HashSet<>(Arrays.asList(tags.split(",")));
            }
            if(TicketUtil.isValidString(agentName) || TicketUtil.isValidCollection(tagSet)) {
    ...
    public void displayAgentTickets()
    {
        ...
        if (TicketUtil.isValidString(agentName))
            displayTicketList(ticketService.ticketsByAgent(agentName));
        else
    ...
    public void displayTicketsByTag()
    {
        ...
        if (TicketUtil.isValidString(tag))
            displayTicketList(ticketService.ticketsByTag(tag));
        else
    ...
    public void totalTicketCount(){
        System.out.println("Total Tickets present in the system : "+ticketService.totalTicketCount());
    }
    ...
    public void ticketOlderByDate(){
        ...
        if(TicketUtil.validDateString(date,"/")) {
            String[] dateArr = date.split("/");
            List<Ticket> tickets = ticketService.ticketOlderByDate(LocalDateTime.of(Integer.parseInt(dateArr[2]), Integer.parseInt(dateArr[1]), Integer.parseInt(dateArr[0]), 0, 0));
            System.out.println("Ticket List ....");
            if (tickets.size() > 0) {
                tickets.stream()
                        .forEach(System.out::println);
            ...
            System.out.println("Invalid date format is entered");
    }
    ...
    public void displayTicketCountByTags(){
        ...
        System.out.println("---------------------------------------------");
        Map<String,Integer> ticketsGroupByTag = ticketService.ticketsGroupByTag();
        if(ticketsGroupByTag.size() > 0)
            ticketsGroupByTag.forEach((String tag,Integer count)->System.out.println(tag+"\t\t"+count));
        else
            System.out.println("No ticket founds");
    }
     ...
     * @param ticketList
     */
    private void displayTicketList(List<Ticket> ticketList){
        if(ticketList.size() > 0) {
            System.out.println(" Ticket list as :- ");
            System.out.println("---------------------------------------------");
